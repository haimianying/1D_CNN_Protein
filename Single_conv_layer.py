...
while(counter != 1):	
	x = Dense(int(counter/4), activation='elu')(x)
	counter = counter/4.0
...
while(counter != maxLen/2):
	x = Dense(int(counter), activation='elu')(x)
	counter = counter*4

x = Dense(int(maxLen/4*nFilters), activation='elu')(x)
x = Reshape((int(maxLen/4),nFilters))(x)
x = UpSampling1D(4)(x)
